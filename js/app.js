// ===========================
// js/app.js — Main Application Logic (with Admin Auth)
// ===========================
import {
  ...
  auth, loginAdmin, logoutAdmin, onAuthChange
} from './db.js';
...
let currentAdmin = null;   // Firebase user หรือ null

// ===========================
// INIT
// ===========================
async function init() {
  // ฟัง auth state ก่อนโหลดข้อมูล
...
function applySettings() {
  ...
  if (!s) return;
  if (s.shop_name) {
    document.querySelectorAll('[data-setting="shop_name"]').forEach(el => el.textContent = s.shop_name);
...
// Guard: ต้อง login ก่อนเข้าหน้า admin
...
export function showProductDetail(id) {
  ...
  const mainImg = imgs.length > 0
    ? `<img src="${imgs[0]}" id="detail-main-img" style="width:100%;height:100%;object-fit:cover;">`
    : `<span id="detail-main-img" style="font-size:64px;">${cat ? cat.icon : '🏷️'}</span>`;
  const thumbs = imgs.length > 1
    ? `<div class="detail-thumbs">${imgs.map((src, i) =>
        `<img src="${src}" class="detail-thumb ${i===0?'active':''}" onclick="app.switchDetailImg('${src}', this)">`
      ).join('')}</div>`
    : '';
  ...
  document.getElementById('detail-body').innerHTML = `
    <div class="product-detail-imgs">
      <div class="product-detail-main">${mainImg}</div>
      ${thumbs}
    </div>
    ...
    <div class="detail-row" style="align-items:flex-start"><div class="detail-key">รายละเอียด</div><div class="detail-val" style="line-height:1.7;font-weight:400;">${(p.description || '-').replace(/\n/g,'<br>')}</div></div>
  `;
  document.getElementById('detail-modal').classList.add('open');
}

export function switchDetailImg(src, thumbEl) {
...
export async function saveProduct() {
  ...
  const catId = document.getElementById('product-cat').value;
...
window.app = {
  ...
  showProductDetail, switchDetailImg, closeDetailModal, inquireProduct, inquireProductId,
  showAdminPanel, handleLogout,
